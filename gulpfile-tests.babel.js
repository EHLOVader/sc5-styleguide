...
tasks = {
    ...
    // 'jscs': runJscs,
    // 'jshint': runJsHint,
    // 'lint:js': gulp.series('jscs', 'jshint'),
    ...
    'generate-coverage-report': generateCoverageReport //jscs:enable disallowQuotedKeysInObjects

};

function srcJsLint() {
    return vfs.src(['gulpfile.babel.js', 'gulpfile-tests.babel.js', 'gulp-tasks/*', 'bin/**/*.js', 'lib/**/*.js', 'test/**/*.js', '!lib/dist/**/*.js', '!lib/app/js/components/**/*.js']);
}

function runJscs() {
    return srcJsLint().pipe(plumber()).pipe(jscs({
        configPath: '.jscsrc'
    }));
}

function runJsHint() {
    return srcJsLint().pipe(plumber()).pipe(jshint()).pipe(jshint.reporter('default')).pipe(jshint.reporter('fail'));
}
...
function runUnitTests(done) {
    vfs.src(['lib/modules/**/*.js']).pipe(istanbul({
        includeUntested: true
    })).pipe(istanbul.hookRequire()).on('finish', () => {
        vfs.src(['test/unit/**/*.js']).pipe(runMocha()).pipe(writeUnitTestCoverage()).pipe(printUnitTestCoverage()).on('end', done);
    });
}
...
function writeUnitTestCoverage() {
    return istanbul.writeReports({
        reporters: ['json'],
        reportOpts: {
            file: path.resolve('./coverage/unit-coverage.json')
        }
    });
}
...
function generateCoverageReport() {
    ...
    return vfs.src('coverage/*.json').pipe(through.obj((file, enc, done) => {
